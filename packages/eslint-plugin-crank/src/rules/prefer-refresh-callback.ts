...
export const preferRefreshCallback: Rule.RuleModule = {
  ...
  create(context) {
     ...
     */
    function isMatchingContext(node: ESLintNode, contextVar: string | null): boolean {
     ...
     */
    function getCallbackParamsText(callbackNode: ESLintNode, sourceCode: Rule.RuleContext["sourceCode"]): string {
    ...
    function createRefreshCallbackFix(
      ...
      return (fixer: Rule.RuleFixer) => {
        ...
        const textAfterCallback = sourceCode.getText().slice(callbackEnd, callbackEnd + 1);
        ...
        if (hasSemicolon) {
          return fixer.replaceTextRange([callbackNode.range![0], callbackEnd + 1], newCallbackText);
        }
