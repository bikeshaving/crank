...
var SPECIAL_PROP_BASES = ["key", "ref", "static"];
var SPECIAL_PROPS = /* @__PURE__ */ new Set(["children", ...SPECIAL_PROP_BASES]);
  ...
  for (let i = 0; i < DEPRECATED_PROP_PREFIXES.length; i++) {
    ...
    for (let j = 0; j < SPECIAL_PROP_BASES.length; j++) {
      ...
      if (deprecatedPropName in props) {
        console.warn(`The \`${deprecatedPropName}\` prop is deprecated. Use \`${propBase}\` instead.`);
        props[propBase] = props[deprecatedPropName];
      }
    }
  }
...
function diffChildren(renderer2, root, host, ctx, scope, parent, children, hydrationData) {
  ...
  for (; oi < oldLength; oi++) {
    ...
    if (typeof ret === "object" && (typeof ret.el.key === "undefined" || !seenKeys || !seenKeys.has(ret.el.key))) {
      (graveyard = graveyard || []).push(ret);
    }
  }
...
function runComponent(ctx, hydrationData) {
  ...
  if (ctx.f & IsSyncGen) {
    ...
    if (ctx.f & IsInForOfLoop && !(ctx.f & NeedsToYield) && !(ctx.f & IsUnmounted)) {
...
async function runAsyncGenComponent(ctx, iterationP, hydrationData, initial = false) {
  ...
  try {
    while (!done) {
      ...
      let value;
      try {
        if (!(ctx.f & NeedsToYield) && ctx.f & PropsAvailable && ctx.f & IsInForAwaitOfLoop && !initial && !done) {
      ...
      if (ctx.ret.inflightValue) {
        oldResult = ctx.ret.inflightValue.then((value2) => ctx.renderer.read(value2));
        oldResult.catch((err) => {
          if (ctx.f & IsUpdating) {
            return;
          }
...
var impl = {
  ...
  hydrate(tag, node, props) {
    ...
    if (typeof tag === "string" && tag.toUpperCase() !== node.tagName) {
      return void 0;
    }
    const children = [];
  ...
  patch(_tag, node, name, value, oldValue, xmlns) {
    ...
    switch (name) {
      ...
      default: {
        ...
        if (name in node && // boolean properties will coerce strings, but sometimes they map to
        // enumerated attributes, where truthy strings ("false", "no") map to
        // falsy properties, so we use attributes in this case.
        !(typeof value === "string" && typeof node[name] === "boolean")) {
          let obj = node;
          do {
          ...
          if (descriptor != null && (descriptor.writable === true || descriptor.set !== void 0)) {
            if (node[name] !== value || oldValue === void 0) {
              node[name] = value;
            }
            return;
          }
    ...
    if (hydrationData != null) {
      if (Array.isArray(result)) {
        for (let i = 0; i < result.length; i++) {
          const node = result[i];
          if (typeof node !== "string" && (node.nodeType === Node.ELEMENT_NODE || node.nodeType === Node.TEXT_NODE)) {
...
@esbuild-plugins/node-globals-polyfill/Buffer.js:
   ...
   *)
*/
//# sourceMappingURL=dom-H5HWQALG.js.map
